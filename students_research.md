---
layout: page
title: 
permalink: /stud_research/
---

<script src="https://polyfill.io/v3/polyfill.min.js?features=es6"></script>
<script id="MathJax-script" async src="https://cdn.jsdelivr.net/npm/mathjax@3/es5/tex-mml-chtml.js"></script>

<h3>For students interested in working with me</h3>

Some points for students to consider before deciding to work with me:

<li> You will need to invest considerable amount of time (ideally a minimum of 1 year).</li>
<li> You will need to have a problem solving mentality.</li>
<li> You will need to have a reasonable exposure to programming (any language is fine).</li>
<li> Don't expect quick fix ``research".</li>
<li> Don't expect me to hand hold you. You are in-charge of the project. I am only a wall to bounce ideas and to guide you. You will have to do the work.</li>
<li> Last but probably the most important point; You should communicate (both written and oral) clearly in English. I am not a psychologist; hence I am not capable of reading your mind.</li>

<h3>For students approaching me for recommendation letters</h3>

I provide recommendation only to students who have done research work with me for a considerable period of time. Though I welcome you to take my class, I am no longer writing reference letters for students who have attended my class.